...
A command-focused plugin starter that shows clean action parsing, runtime diagnostics, and small stateful examples you can expand into real tools.

## Highlights
- root command registration with readable action handling
- status output that is useful while iterating locally
- small mutable state that makes command flows easy to follow
...
Built jars are written to `build/libs/hytale-plugin-template-commands-1.1.0.jar`, with matching sources and javadoc jars next to it.

## Commands
- `/hdcommandsdemo`: Runs a demo action for the Commands template.
- `/hdcommandsstatus`: Shows runtime status for the Commands template.
- Common actions: `info, toggle, sample, alias-check, register-alias, resolve-alias`
...
2. Copy `build/libs/hytale-plugin-template-commands-1.1.0.jar` into your server `plugins/` directory.
