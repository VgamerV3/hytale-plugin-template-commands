...
public final class CommandsStatusCommand extends AbstractCommand {
  ...
  public CommandsStatusCommand(
      CommandsPluginState state,
      CommandsDemoService demoService,
      AtomicLong heartbeatTicks,
      LongSupplier uptimeSeconds,
      BooleanSupplier heartbeatActive) {
    super("hdcommandsstatus", "Shows runtime status for the Commands template.");
  ...
  protected CompletableFuture<Void> execute(CommandContext ctx) {
    ...
    ctx.sendMessage(Message.raw("[Commands] sender=" + sender + ", lastAction=" + demoService.describeLastAction(sender) + ", " + demoService.diagnostics()));
